...
export class Gratis {
  ...
  async searchGratis(
    ...
    try {
      ...
      await page.goto(
        `https://www.gratis.com/searchresults?N=3790920594&Ntt=${searchKey}&No=0&Rdm=288&Nr=product.active:1`,
        {
      ...
      let brand_list = await page.$$(
        ".occ-facet-display-filter > div > .content > div"
      );
...
async function getProducts(product_cards: any, res) {
  ...
  let productPromise = (product_card) =>
    new Promise(async (resolve, reject) => {
      ...
      let price_info = await product_card.$(
        ".occ-product-gratis-card-new-wrapper"
      );
      try {
        let price_amount = await price_info.$eval(
          ".gr-price__amount",
          (text) => text.textContent
        );
        let price_fractional = await price_info.$eval(
          ".gr-price__fractional",
          (text) => text.textContent
        );
        data["price"] =
          parseFloat(price_amount) +
          parseFloat(price_fractional.replace(",", "0."));
        data["currency"] = await product_card.$eval(
          ".gr-price__currency",
          (text) => text.textContent
        );
      } catch (error) {
