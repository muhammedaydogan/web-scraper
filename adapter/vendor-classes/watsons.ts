...
export class Watsons {
  async searchWatsons(
    ...
    try {
      ...
      await page.goto(
        `https://www.watsons.com.tr/search?q=${searchKey}&personaclick_search_query=${searchKey}&personaclick_input_query=${searchKey}&specs=${categoryId},${brandId}`,
        {
          waitUntil: "networkidle2",
        }
      );
      let category_list = await page.$$(
        ".filter-item.kategori > div > ul > li"
      );
...
async function getProducts(product_cards: any, res) {
  res.data = [] as any;
  let productPromise = (product_card) =>
    new Promise(async (resolve, reject) => {
      let data = {};
      data[
        "brand"
      ] = await product_card.$eval(
        ".productbox-desc.text-left.mb-1 > span",
        (text) => text.innerHTML.replace(/(\r\n\t|\n|\r|\t)/gm, "").trim()
      );
      data["title"] = await product_card.$eval(
        ".productbox-desc.text-left.mb-1",
        (text) => text.innerHTML
      );
      data["title"] = data["title"]
        .substr(data["title"].lastIndexOf("</span>\n") + 8)
        .trim();
      let dom = parser.parseFromString(data["title"]);
      // console.log(dom.children);

      let price = await product_card.$eval(
        ".list-inline-item.product-box-price.text-site-pink.roboto-medium",
      ...
      data["image_link"] = await product_card.$eval(
        "a > img",
        (text) => text.src
      );
      resolve(data);
